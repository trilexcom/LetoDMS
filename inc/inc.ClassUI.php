...
class UI {
	...
	function globalBanner() {
		...
		echo "<div class=\"siteName\">".
			(strlen($settings->_siteName)>0 ? $settings->_siteName : "MyDMS").
			"</div>\n";
	...
	function folderNavigationBar($folder) {
		...
		if ($accessMode == M_READ && $user->getID() != $settings->_guestID) {
			...
			echo "<li><a href=\"../out/out.FolderNotify.php?folderid=". $folderID ."\">".getMLText("edit_folder_notify")."</a></li>\n";
	...
	function documentNavigationBar()	{
		...
		echo "<ul class=\"localNav\">\n";
		if ($accessMode == M_READ && $user->getID() != $settings->_guestID) {
			echo "<li id=\"first\"><a href=\"../out/out.DocumentNotify". $docid ."\">".getMLText("edit_document_notify")."</a></li>";
		}
		else if ($accessMode >= M_READWRITE) {
			if (!$document->isLocked()) {
				...
				echo "<li><a href=\"../out/out.DocumentNotify". $docid ."\">".getMLText("edit_document_notify")."</a></li>";
	...
	function pageList($pageNumber, $totalPages, $baseURI, $params) {
		...
		if (!is_numeric($pageNumber) || !is_numeric($totalPages) || $totalPages<1) {
			return;
		}
		...
		echo "<li id=\"first\">".getMLText("results_page").": ";
		for ($i = 1; $i  <= $totalPages; $i++) {
			if ($i == $pageNumber) {
				echo "<li>".$i;
			}
			else {
				echo "<li><a href=\"".$resultsURI.($first ? "?" : "&")."pg=".$i."\">".$i."</a>";
			}
		}
		if ($totalPages>1) {
			echo "<li><a href=\"".$resultsURI.($first ? "?" : "&")."pg=all\">".getMLText("all_pages")."</a>";
		}
		echo "</ul>";
	...
	function printDocumentChooser($formName) {
		...
		print "&nbsp;&nbsp;<input type=\"Button\" value=\"".getMLText("search")."...\" onclick=\"chooseDoc();\">";
	}
	...
	function printFolderChooser($formName, $accessMode, $exclude = -1, $default = false) {
		...
		var openDlg;
		function chooseDoc() {
			openDlg = open("out.FolderChooser.php?form=<?php echo $formName?>&mode=<?php echo $accessMode?>&exclude=<?php echo $exclude?>&folderid=<?php echo $settings->_rootFolderID?>", "openDlg", "width=480,height=480,scrollbars=yes,resizable=yes,status=yes");
		}
		</script>
		<?php
